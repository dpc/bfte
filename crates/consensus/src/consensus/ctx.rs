...
impl<T> InsertOutcome<T> {
    pub(crate) fn map<O>(self, f: impl FnOnce(T) -> O) -> InsertOutcome<O> {
        match self {
    ...
    ($t:ty) => {
        impl ConsensusReadDbOps for $t {
            ...
            fn get_vote_dummy(
                ...
                let tbl = self.open_table(&tables::cons_votes_dummy::TABLE)?;
            ...
            fn get_vote_block(
                ...
                let tbl = self.open_table(&tables::cons_votes_block::TABLE)?;
            ...
            fn get_peers_with_proposal_votes(&self, round: BlockRound) -> DbResult<VoteSet> {
                ...
                let tbl_votes_block = self.open_table(&tables::cons_votes_block::TABLE)?;
            ...
            fn get_peers_with_dummy_votes(&self, round: BlockRound) -> DbResult<VoteSet> {
                ...
                let tbl_votes_dummy = self.open_table(&tables::cons_votes_dummy::TABLE)?;
            ...
            fn get_consensus_params_opt(
                ...
                let Some(hash) = self
                    .open_table(&tables::cons_params_schedule::TABLE)?
                    .range(..=round)?
                    .next_back()
                    .transpose()?
                    .map(|g| g.1.value())
                ...
                let params = self
                    .open_table(&tables::cons_params::TABLE)?
                    .get(&hash)?
                    .map(|g| g.value())
            ...
            fn get_finality_vote(&self, peer_pubkey: PeerPubkey) -> DbResult<Option<BlockRound>> {
                let tbl = self.open_table(&tables::cons_finality_votes::TABLE)?;
            ...
            fn get_block_payload(
                ...
                let tbl = self.open_table(&tables::cons_blocks_payloads::TABLE)?;

                Ok(tbl.get(&payload_hash)?.map(|g| g.value()))
            }

            fn get_proposal(&self, round: BlockRound) -> DbResult<Option<BlockHeader>> {
                let tbl = self.open_table(&tables::cons_blocks_proposals::TABLE)?;
            ...
            fn has_notarized_non_dummy_block(&self, round: BlockRound) -> DbResult<bool> {
                let tbl = self.open_table(&tables::cons_blocks_notarized::TABLE)?;
            ...
            fn get_notarized_block(&self, round: BlockRound) -> DbResult<Option<BlockHeader>> {
                let tbl_blocks = self.open_table(&tables::cons_blocks_notarized::TABLE)?;

                Ok(tbl_blocks.get(&round)?.map(|v| v.value()))
            }

            fn get_pinned_block(&self, round: BlockRound) -> DbResult<Option<BlockHash>> {
                let tbl = self.open_table(&tables::cons_blocks_pinned::TABLE)?;
...
impl ConsensusWriteDbOps for WriteTransactionCtx {
    fn insert_consensus_params(
        ...
        let mut tbl_schedule = self.open_table(&tables::cons_params_schedule::TABLE)?;
        let mut tbl = self.open_table(&tables::cons_params::TABLE)?;

        let hash = params.hash();

        if let Some(existing) = tbl_schedule.insert(&round, &hash)?.map(|v| v.value()) {
            debug_assert_eq!(existing, hash);
    ...
    fn insert_block_proposal(
        ...
        let mut tbl_blocks = self.open_table(&tables::cons_blocks_proposals::TABLE)?;
        ...
        let mut tbl_blocks_payloads = self.open_table(&tables::cons_blocks_payloads::TABLE)?;
        tbl_blocks_payloads.insert(&block.payload_hash, payload)?;

        let mut tbl_votes_blocks = self.open_table(&tables::cons_votes_block::TABLE)?;
    ...
    fn update_finality_consensus(&self, round: BlockRound) -> DbResult<Option<BlockRound>> {
        let mut tbl = self.open_table(&tables::cons_finality_consensus::TABLE)?;
    ...
    fn prune_dummy_votes(&self, round: BlockRound) -> DbResult<()> {
        let mut tbl = self.open_table(&tables::cons_votes_dummy::TABLE)?;
    ...
    fn update_finality_vote(
        ...
        let mut tbl = self.open_table(&tables::cons_finality_votes::TABLE)?;
    ...
    fn insert_notarized_block(
        ...
        let mut tbl_blocks = self.open_table(&tables::cons_blocks_notarized::TABLE)?;
        if let Some(existing) = tbl_blocks.get(&round)?.map(|x| x.value()) {
    ...
    fn insert_pinned_block(
        ...
        let mut tbl = self.open_table(&tables::cons_blocks_pinned::TABLE)?;
    ...
    fn insert_block_vote(
        ...
        let mut tbl = self.open_table(&tables::cons_votes_block::TABLE)?;
    ...
    fn set_current_round(&self, round: BlockRound) -> DbResult<()> {
        let mut tbl = self.open_table(&tables::cons_current_round::TABLE)?;
        tbl.insert(&(), &round)?;
        Ok(())
    }
}
