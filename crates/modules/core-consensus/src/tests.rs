...
impl TestSetup {
    async fn bootstrap_single_peer() -> BoxedErrorResult<Self> {
        ...
        let module_config = module_db.write_with_expect(|module_dbtx| {
        ...
        let module_init = CoreConsensusModuleInit;
        let module = module_init.init(ModuleInitArgs::new(
            module_id,
            db.clone(),
            CURRENT_VERSION,
    ...
    /// Downcast the module to CoreConsensusModule for accessing concrete implementation methods
...
async fn test_vote_add_peer_produces_effect() -> BoxedErrorResult<()> {
    ...
    let effects = setup.core_module().db.write_with_expect_falliable(|dbtx| {
    ...
    let decoded_effect: AddPeerEffect = EffectKindExt::decode(effect)
        .map_err(|e| format!("Failed to decode effect: {e}"))?;
    ...
    // Use the module's get_peer_set method instead of poking at the database directly
...
async fn test_two_peers_voting_to_add_third() -> BoxedErrorResult<()> {
    ...
    let effects1 = setup.core_module().db.write_with_expect_falliable(|dbtx| {
        setup.module.process_citem(
            dbtx,
            BlockRound::from(0),
            peer1_pubkey,
    ...
    assert_eq!(effects1.len(), 0, "No effects should be produced with only one vote");
    
    // Verify that the new peer was NOT added yet
    let peer_set_after_first_vote = setup.core_module().get_peer_set().await;
    assert!(!peer_set_after_first_vote.contains(&new_peer_pubkey), "New peer should NOT be added after first vote");
    assert_eq!(peer_set_after_first_vote.len(), 2, "Peer set should still contain exactly 2 peers");
    
    // Second peer votes to add the new peer - should reach threshold now
    let effects2 = setup.core_module().db.write_with_expect_falliable(|dbtx| {
        setup.module.process_citem(
            dbtx,
            BlockRound::from(0),
            peer2_pubkey,
    ...
    assert_eq!(effects2.len(), 1, "Expected exactly one effect after reaching threshold");
